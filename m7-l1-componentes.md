...
Objetivos
• Comprender la estructura y jerarquía de los componentes
...
3. Paso de Props y emisión de eventos
    ...
    • Desde Padre.vue
    pasa un mensaje como prop "Hola desde padre"
    componente Hijo.vue

    • En Hijo.vue, crea un botón que, al hacer clic,
    emita un evento llamado respuestaHijo
    y envíe un mensaje de respuesta al padre.

    • En Padre.vue, captura el evento respuestaHijo y
    muestra el mensaje con v-if.

4. Hooks del ciclo de vida
    En Padre.vue utiliza
    • mounted
    • updated
    • beforeUnmount

    escribe un mensaje en la consola para cada uno
...
llamarse componentes-practica y contener los archivos relevantes en su estructura original.
(No es necesario incluir la carpeta node_modules ni el archivo package-lock.json para reducir
el tamaño del comprimido).
• Captura de pantalla de la consola del navegador, donde se vean los mensajes registrados por
los hooks del ciclo de vida (mounted, updated, beforeUnmount, etc.).
• Archivo word, que incluya una breve explicación de los siguientes puntos:
• se pasan los datos con props.
• Cómo se emiten eventos con $emit.
• Qué hace cada hook del ciclo de vida utilizado.
