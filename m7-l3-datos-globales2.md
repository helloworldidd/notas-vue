...
router.beforeEach((to) => {
  ...
  if (requiereAuth && !authStore.isLogged) {
    return {
      name: 'Login'
    }
  }
...
<template>
  <section>
    ...
    <form @submit.prevent="formStore.registrar">
      <input
        v-model="formStore.form.nombre"
        placeholder="Nombre"
      />

      <input
        v-model="formStore.form.apellidos"
      ...
      <select v-model="formStore.form.transporte">
      ...
      <input
        v-if="formStore.mostrarMatricula"
        v-model="formStore.form.matricula"
    ...
    <div
      v-if="formStore.errores.length"
      style="color: red;"
    >
      <p
        v-for="error in formStore.errores"
        :key="error"
      >
        {{ error }}
      </p>
